...
instead, Schmitt-triggered with the usual ~1 V/2 V hysteresis. **CLOCK OUT** mirrors
...
| **BITS** | Picks which contiguous 8 of the register's 18 bits DAC OUT reads, as a window offset 0–10. |
| **RND** (panel: RND OUT) | The register's own feedback bit each clock, as ±5 V — white-ish noise at an audio-rate clock, a random gate held between edges at a slow one. |
| **DAC** (panel: DAC OUT) | The 8-bit window BITS selects, read as an unsigned value and scaled to a stepped 0–10 V random CV. A new step lands on every clock edge. |

## SAMPLE & HOLD

| Control/jack | Role |
|---|---|
| **SRC** (panel: SRC IN) | What gets sampled. Normalled to the module's own white noise (the same signal NOISE OUT carries) when unpatched — polyphonic, so a patched multi-channel source samples every channel independently. |
| **TRIG** (panel: TRIG IN) | When to sample. Normalled to the shared clock when unpatched. |
| **SLEW** | A trim that glides the stepped output rather than jumping to it — 0 is instant, full CW is about 250 ms to settle. |
| **S&H** (panel: OUT) | The held (and optionally slewed, optionally drooping) voltage. |

## RND GATE

| Control/jack | Role |
|---|---|
| **PROB** | The odds, 0–100%, that GATE OUT goes high on the next shared clock edge. |
...
| **GATE** (panel: GATE OUT) | High for the full clock period whenever a draw succeeds (default), or see the TOGGLE menu option below. |
| **NOISE** (panel: NOISE OUT) | The module's continuous white noise, the same signal SAMPLE & HOLD's SRC and RND GATE's SRC IN both normal to. |

## Voltage conventions

- CLOCK IN/OUT: 0/10 V gate convention, ~1 V (CLOCK IN) or 2 V (CLOCK IN, high
  threshold) Schmitt hysteresis; CLOCK OUT retriggers a 1 ms pulse per external edge,
  or mirrors the internal square wave's own 50% duty.
- RND OUT, SRC IN, NOISE OUT: bipolar, roughly ±5 V.
- DAC OUT, SAMPLE & HOLD OUT: unipolar/whatever SRC IN carries — DAC OUT is fixed at
  0–10 V.
- GATE OUT: 0/10 V.
...
- **Sample & hold droop** — the held voltage leaks back toward 0 V with a roughly
  ...
  default.
- **Random gate: toggle mode** — GATE OUT flips its state on a successful draw
...
SAMPLE & HOLD is polyphonic, following SRC IN's channel count (falling back to 1
channel when SRC IN is unpatched and it is sampling the module's own mono white
...
- RND GATE's "Ext" source and SAMPLE & HOLD's SRC both normal to the same internally
  ...
  available on RND OUT and DAC OUT for patching in by hand.
